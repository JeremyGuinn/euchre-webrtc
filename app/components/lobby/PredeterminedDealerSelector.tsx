        ...
        <div className='text-center'>
          ...
          {selectedPlayer ? (
            <p className='text-gray-600'>
              <span className='font-medium'>{selectedPlayer.name}</span> will be
              the first dealer
            </p>
          ) : (
            <p className='text-gray-500'>
              Host is selecting the first dealer...
            </p>
          )}
        </div>
      </Panel>
      ...
      {selectedDealerId && (
        <div className='mt-4 p-3 bg-blue-50 rounded-lg'>
          <p className='text-sm text-blue-700'>
            <strong>
              {players.find(p => p.id === selectedDealerId)?.name}
            </strong>{' '}
            will be the first dealer when the game starts.
          </p>
        </div>
      )}
    </Panel>
