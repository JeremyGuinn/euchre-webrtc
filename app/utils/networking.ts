...
export class NetworkManager {
  ...
  private messageHandlers = new Map<
    string,
    (message: GameMessage, senderId: string) => void
  >();
      ...
      if (targetId) {
        ...
        if (connection?.connection.open) {
          try {
            ...
            console.warn(`Failed to send message to ${targetId}, connection may be closed:`, error);
            this.handleConnectionError(targetId);
          }
        } else if (connection) {
          // Connection exists but is not open, handle as disconnected
          console.warn(`Attempted to send message to ${targetId} but connection is not open`);
