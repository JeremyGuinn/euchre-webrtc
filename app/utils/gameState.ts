    ...
    switch (action.type) {
      ...
      case 'RECONNECT_PLAYER': {
        ...
        const updatedState: GameState = {
          ...
          bids: state.bids.map(bid =>
            bid.playerId === oldPlayerId
              ? { ...bid, playerId: newPlayerId }
              : bid
          ),
      ...
      case 'START_GAME':
        ...
        if (state.options.dealerSelection === 'predetermined_first_dealer') {
          const predeterminedDealerId =
            state.options.predeterminedFirstDealerId;
      ...
      case 'DEAL_CARDS': {
        ...
        let currentPlayerId = getNextPlayer(
          state.currentDealerId,
          state.players
        );
      ...
      case 'SET_TRUMP':
        return {
          ...
          maker: {
            ...
            teamId:
              state.players.find(p => p.id === action.payload.makerId)
                ?.teamId || 0,
